...
fn test_tcp_connected_state_invariants(
    net: tcp::Network,
    sock: tcp::TcpSocket,
    ...
    assert!(matches!(
        tcp::start_connect(sock, net, addr),
    ...
    assert!(matches!(tcp::remote_address(sock), Ok(_)));
...
fn main() {
    ...
    {
        let sock_listener = tcp_create_socket::create_tcp_socket(IpAddressFamily::Ipv4).unwrap();
        tcp_blocking_bind(
            sock_listener,
            net,
        ...
        .unwrap();
        tcp_blocking_listen(sock_listener).unwrap();

        let addr_listener = tcp::local_address(sock_listener).unwrap();

        let sock_client = tcp_create_socket::create_tcp_socket(IpAddressFamily::Ipv4).unwrap();
        tcp_blocking_connect(sock_client, net, addr_listener).unwrap();

        test_tcp_connected_state_invariants(net, sock_client, IpAddressFamily::Ipv4, addr_listener);
    }
    {
        let sock_listener = tcp_create_socket::create_tcp_socket(IpAddressFamily::Ipv6).unwrap();
        tcp_blocking_bind(
            sock_listener,
            net,
        ...
        let sock_client = tcp_create_socket::create_tcp_socket(IpAddressFamily::Ipv6).unwrap();
        tcp_blocking_connect(sock_client, net, addr_listener).unwrap();

        test_tcp_connected_state_invariants(net, sock_client, IpAddressFamily::Ipv6, addr_listener);
    }
}
